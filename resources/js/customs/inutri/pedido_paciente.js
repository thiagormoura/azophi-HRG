$(document).ready(() => {
  var selectUnidade = $('#inutri-select-unidade').selectize({
    create: false,
    sortField: "text",
    persist: false,
    onChange: function (unidade) {
      if (unidade !== '') {
        $.ajax({
          url: `getPacientes/unidade/${unidade}`,
          type: "POST",
          dataType: "html",
        }).done(function (responseText) {
          $('.container-select-paciente').html(responseText);
          var selectPaciente = $('#inutri-select-paciente').selectize({
            create: false,
            sortField: "text",
            persist: false,
            onChange: function (registro) {
              if (registro !== '') {
                $.ajax({
                  url: `getPacientes/dieta/${registro}`,
                  type: "POST",
                  dataType: "html",
                }).done(function (responseText) {
                  $('.container-cardapio').html(responseText);
          ...
          var selectizePaciente = selectPaciente[0].selectize;
          selectizePaciente.setValue('', false);
        }).fail(function (jqXHR, textStatus) {
          console.log("Request failed: " + textStatus);
        }).always(function () { });
      }
