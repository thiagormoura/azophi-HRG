...
class Setor
{
  ...
  {
    $this->id = (new Database('centralservicos', 'setor'))->insert([
      'codigo' => $this->codigo,
      'nome' => $this->nome,
      'status' => $this->status,
    ]);
  ...
  {
    return (new Database('centralservicos', 'setor'))->update("id = " . $this->id,  [
      'codigo' => $this->codigo,
      'nome' => $this->nome,
      'status' => $this->status,
    ]);
  ...
  public static function getSetores(){
    return (new Database('centralservicos', 'setor'))->select('*', "`status` = 'A'")->fetchAll(\PDO::FETCH_CLASS, self::class);
  }

  // Método responsável por retornar um setor pelo código
  public static function getSetorByCodigo($codigo){
    return (new Database('centralservicos', 'setor'))->select('*', "codigo = '" . $codigo . "'")->fetchObject(self::class);
  }
}
